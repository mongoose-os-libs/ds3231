...
struct mgos_ds3231_date_time* mgos_ds3231_date_time_create() {
  struct mgos_ds3231_date_time* dt = (struct mgos_ds3231_date_time*) calloc(1, sizeof (struct mgos_ds3231_date_time));
  if (NULL == dt) {
...
const struct mjs_c_struct_member* mgos_ds3231_date_time_get_struct_descr() {
  static const struct mjs_c_struct_member struct_descr[] = {
    {"Second", offsetof(struct mgos_ds3231_date_time, Second), MJS_FFI_CTYPE_INT},
    {"Minute", offsetof(struct mgos_ds3231_date_time, Minute), MJS_FFI_CTYPE_INT},
    ...
    {"Month", offsetof(struct mgos_ds3231_date_time, Month), MJS_FFI_CTYPE_INT},
    {"Year", offsetof(struct mgos_ds3231_date_time, Year), MJS_FFI_CTYPE_INT},
    {"unixtime", offsetof(struct mgos_ds3231_date_time, unixtime), MJS_FFI_CTYPE_INT},
...
struct mgos_ds3231* mgos_ds3231_create(uint8_t addr) {
  ...
  struct mgos_ds3231* ds = (struct mgos_ds3231*) calloc(1, sizeof (struct mgos_ds3231));
  if (NULL == ds) {
...
bool mgos_ds3231_write(struct mgos_ds3231* ds, const struct mgos_ds3231_date_time* current_date) {
  if (NULL == ds) {
    return false;
  }
  ...
  data[3] = bin2bcd(current_date->Dow ? current_date->Dow : 1); // People might not bother with Dow, make sure it's valid, in case.
...
bool mgos_ds3231_disable_alarms(struct mgos_ds3231* ds) {
  ...
  mgos_ds3231_set_alarm(ds, dt, MGOS_DS3231_ALARM_MATCH_SECOND_MINUTE_HOUR_DATE);
...
uint8_t mgos_ds3231_check_alarms(struct mgos_ds3231* ds) {
  ...
  if (alarm) {
    // Clear the alarm
    mgos_i2c_write_reg_b(ds->_i2c, ds->_addr, DS3231_REG_STATUS, status_byte & ~0x3);
  }
...
uint8_t mgos_ds3231_set_alarm(struct mgos_ds3231* ds, const struct mgos_ds3231_date_time* alarm_date, uint8_t alarm_mode) {
  ...
  uint8_t control_byte = mgos_i2c_read_reg_b(ds->_i2c, ds->_addr, DS3231_REG_CONTROL);
  ...
  if (alarm_mode & 0B00000001) /* Alarm 1 Modes */ {
    ...
    control_byte = control_byte | _BV(0) | _BV(2); // Enable Alarm 1, set interrupt output on alarm.
  } else {
    // Start address of data for Alarm2
    data[i++] = DS3231_REG_ALARM_2;
    control_byte = control_byte | _BV(1) | _BV(2); // Enable Alarm 2, set interrupt output on alarm.
  }
